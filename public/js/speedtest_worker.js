...
var settings = {
	...
	url_dl: "garbage", // path to a large file or garbage, used for download test. must be relative to this js file
	...
	dlOverheadCompensationFactor: 0.973, //can be changed to compensatie for transport overhead. (see doc.md for some other values)
	ulOverheadCompensationFactor: 1.00, //can be changed to compensatie for transport overhead. (see doc.md for some other values)
	...
	interval = setInterval(
		function() {
			...
			if (!graceTimeDone) {
				...
				//update status
				dlStatus = ((speed * 8 * settings.dlOverheadCompensationFactor) / (settings.useMebibits ? 1048576 : 1000000)).toFixed(2); // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 or 1000000 to go to megabits/mebibits
		...
		interval = setInterval(
			function() {
				...
				if (!graceTimeDone) {
					...
					//update status
					ulStatus = ((speed * 8 * settings.ulOverheadCompensationFactor) / (settings.useMebibits ? 1048576 : 1000000)).toFixed(2); // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 or 1000000 to go to megabits/mebibits
